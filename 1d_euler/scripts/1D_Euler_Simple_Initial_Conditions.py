...
def get_model(core_params, model_params):
    ...
    return LearnedFlux(
        features, model_params.kernel_size, model_params.kernel_out, core_params.bc
    )
...
for i, nx in enumerate(nxs):
    ...
    losses, params = train_model(
        model, i_params, training_params(nx), key_train, idx_fn, batch_fn, loss_fn
    )
...
for n in range(N_test):
    ...
    for i, nx in enumerate(nxs):
        ...
        # exact trajectory
        trajectory_exact_ds = convert_trajectory_fn(
            trajectory_exact, nx, core_params_exact.Lx
        )
        ...
        trajectory_invariant_ML = get_trajectory_ML(
            a0, aL, aR, params, invariant_preserving=True, cfl_safety=0.05
        )
