...
def flux_laxfriedrichs(aL, aR, core_params, dt, dx):
    return 0.5 * (f_j(aL, core_params) + f_j(aR, core_params)) - 0.5 * (dx / dt) * (
        aR - aL
    )


def flux_rusanov(aL, aR, core_params):
    local_max_speed = jnp.maximum(
        jnp.abs(get_u(aL, core_params)) + get_c(aL, core_params),
        (jnp.abs(get_u(aR, core_params)) + get_c(aR, core_params)),
    )
    return 0.5 * (
        f_j(aL, core_params) + f_j(aR, core_params)
    ) - 0.5 * local_max_speed * (aR - aL)
...
def flux_roe(aL, aR, core_params):
    def entropy_fix(eig, eigL, eigR):
        ...
        abs_eig = jnp.abs(eig)
        return (abs_eig >= delta) * abs_eig + (
            abs_eig < delta
        ) * delta  # jnp.nan_to_num(0.5 * (delta + eig**2 / delta))
    ...
    uRoe = (
        jnp.sqrt(rhoL) * get_u(aL, core_params)
        + jnp.sqrt(rhoR) * get_u(aR, core_params)
    ) / denom
    HRoe = (
        jnp.sqrt(rhoL) * get_H(aL, core_params)
        + jnp.sqrt(rhoR) * get_H(aR, core_params)
    ) / denom
    ...
    return 0.5 * (f_j(aL, core_params) + f_j(aR, core_params)) - 0.5 * (
        corr1 + corr2 + corr3
    )
...
def flux_musclcharacteristic_ghost(a, core_params):
    ...
    alpha_1_minus = (b / c**2) * (
        (h - u**2) * dQ_minus[0] + u * dQ_minus[1] - dQ_minus[2]
    )
    alpha_2_minus = (
        1 / (2 * c) * (dQ_minus[1] + (c - u) * dQ_minus[0] - c * alpha_1_minus)
    )
    alpha_0_minus = dQ_minus[0] - alpha_1_minus - alpha_2_minus
    dD_minus = jnp.asarray(
        [alpha_0_minus, alpha_1_minus, alpha_2_minus]
    )  # D stands for Delta = L(Q_i) (Q_{...}-Q_{...})

    alpha_1_plus = (b / c**2) * (
        (h - u**2) * dQ_plus[0] + u * dQ_plus[1] - dQ_plus[2]
    )
...
def _time_derivative_euler_periodic(core_params, model=None, params=None, dt_fn=None):
    if core_params.flux == Flux.MUSCLCONSERVED:
        ...
        flux_fn = lambda aL, aR, core_params: flux_laxfriedrichs(
            aL, aR, core_params, dt_fn(aL), core_params.Lx / aL.shape[1]
        )
        ...
        def flux_term(a):
            ...
            delta_flux_right = flux_learned_periodic(
                a, core_params, model=model, params=params
            )
...
def _time_derivative_euler_ghost(core_params, model=None, params=None, dt_fn=None):
    if core_params.flux == Flux.MUSCLCONSERVED:
        ...
        def flux_term(a):
            ...
            delta_flux_right = flux_learned_nonperiodic(
                a, core_params, model=model, params=params
            )
            return flux_right + delta_flux_right
...
def _time_derivative_euler_open(core_params, model=None, params=None, dt_fn=None):
    if core_params.flux == Flux.MUSCLCHARACTERISTIC:
        flux_term = lambda a, aL, aR: flux_musclcharacteristic_open(
            a, aL, aR, core_params
        )
    elif core_params.flux == Flux.LEARNED:
...
def positivity_limiter_periodic(a, flux_right, core_params, dt_fn):
    ...
    def get_a_plus_minus(a, flux_right, core_params, delta):
        return a - 2 * delta * flux_right, jnp.roll(
            a + 2 * delta * jnp.roll(flux_right, 1, axis=-1), -1, axis=-1
        )
    ...
    flux_fn = lambda aL, aR, core_params: flux_laxfriedrichs(
        aL, aR, core_params, dt_fn(a), core_params.Lx / a.shape[1]
    )
    ...
    flux_right_star = (
        theta_rho * jnp.nan_to_num(flux_right) + (init_ones - theta_rho) * flux_right_LF
    )
...
def positivity_limiter_nonperiodic(a, flux_right, core_params, dt_fn):
    def get_a_plus_minus(a, flux_right, core_params, delta):
        ...
        return (a - 2 * delta * flux_right[:, 1:])[:, :-1], (
            a + 2 * delta * flux_right[:, :-1]
        )[:, 1:]
    ...
    a_LF_plus, a_LF_minus = get_a_plus_minus(
        a, flux_right_LF, core_params, delta
    )  # (3, nx-1)
    rho_LF_plus = a_LF_plus[0]
    rho_LF_minus = a_LF_minus[0]
    p_LF_plus = get_p(a_LF_plus, core_params)
    ...
    theta_plus = theta_plus.at[1:-1].set(
        below_zero * theta_plus_below + above_zero * init_ones
    )
    ...
    theta_minus = theta_minus.at[1:-1].set(
        below_zero * theta_minus_below + above_zero * init_ones
    )

    theta_rho = jnp.minimum(theta_plus, theta_minus)  # (3, nx + 1)

    flux_right_star = (
        theta_rho * jnp.nan_to_num(flux_right) + (1 - theta_rho) * flux_right_LF
    )  # (3, nx + 1)
...
def entropy_increase_ghost(a, flux_right, core_params):
    ...
    diff_w = w[:, 1:] - w[:, :-1]
    deta_dt_old = (
        jnp.sum(flux_right[:, 1:-1] * diff_w)
        + jnp.sum(flux_right[:, 0] * w[:, 0])
        - jnp.sum(flux_right[:, -1] * w[:, -1])
    )
    deta_dt_new = get_entropy_flux(a[:, 0], core_params) - get_entropy_flux(
        a[:, -1], core_params
    )
    denom = jnp.sum(G_R * diff_w)
    flux_right = flux_right.at[:, 1:-1].add(
        jnp.nan_to_num(
            (deta_dt_old < deta_dt_new) * (deta_dt_new - deta_dt_old) * G_R / denom
        )
    )
    return flux_right
...
def entropy_increase_open(a, flux_right, core_params, aL, aR):
    ...
    diff_w = w[:, 1:] - w[:, :-1]
    deta_dt_old = jnp.sum(
        flux_right[:, 1:-1] * diff_w
    )  # + jnp.sum(flux_right[:,0] * w[:,0]) - jnp.sum(flux_right[:,-1] * w[:,-1])
    deta_dt_new = jnp.minimum(
        get_entropy_flux(aL, core_params), get_entropy_flux(a[:, 0], core_params)
    ) - jnp.maximum(
        get_entropy_flux(aR, core_params), get_entropy_flux(a[:, -1], core_params)
    )
    denom = jnp.sum(G_R * diff_w)
    flux_right = flux_right.at[:, 1:-1].add(
        jnp.nan_to_num(
...
def time_derivative_FV_1D_euler(
    core_params, model=None, params=None, dt_fn=None, invariant_preserving=False
):
    if core_params.bc == BoundaryCondition.GHOST:
        flux_term = _time_derivative_euler_ghost(
            core_params, model=model, params=params, dt_fn=dt_fn
        )
        ...
        flux_term = _time_derivative_euler_open(
            core_params, model=model, params=params, dt_fn=dt_fn
        )
        ...
        flux_term = _time_derivative_euler_periodic(
            core_params, model=model, params=params, dt_fn=dt_fn
        )
        ...
        def dadt(a, aL=None, aR=None):
            ...
            if invariant_preserving == True:
                assert dt_fn is not None
                flux_right = positivity_limiter_periodic(
                    a, flux_right, core_params, dt_fn
                )
