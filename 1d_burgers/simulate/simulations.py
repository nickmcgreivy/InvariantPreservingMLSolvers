...
class BurgersFVSim():
	...
	def __init__(self, core_params, sim_params, global_stabilization = False, epsilon_gs = 0.0, G = lambda f, u: jnp.roll(u, -1) - u, model=None, params=None, delta=True, omega_max = 0.0):
		self.global_stabilization = global_stabilization
		self.epsilon_gs = epsilon_gs
		self.model = model
		self.params = params
		self.G = G
		self.step_fn = self.get_step_fn(core_params, sim_params, model=model, params=params, delta=delta)
	...
	def get_step_fn(self, core_params, sim_params, model, params, delta):
		self.F = time_derivative_FV_1D_burgers(core_params, global_stabilization=self.global_stabilization, G=self.G, epsilon_gs=self.epsilon_gs, model=model, params=params, delta=delta)
