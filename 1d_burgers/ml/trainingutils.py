...
def save_training_data(
    ...
    **kwargs
    ...
    create_training_data(
        sim_params, core_params, nxs, outer_steps * n_runs, delta=delta
    )
    ...
    for n in range(n_runs):
        ...
        rollout_fn = jax.jit(
            get_trajectory_fn(inner_fn, outer_steps, start_with_input=True)
        )

        trajectory_fn_warmup = jax.jit(
            get_trajectory_fn(inner_fn, outer_steps_warmup, start_with_input=False)
        )
        ...
        time_derivative_fn = jax.vmap(
            jax.jit(lambda a, t: sim.F(a, t, forcing_func=f_forcing))
        )
        ...
        for j, nx in enumerate(nxs):
            ...
            if delta:
                dadt_to_store = (
                    dadt_trajectory_exact_ds - dadt_trajectory_ds
                )  # delta_dadt_trajectory
            else:
...
def get_batch_fn(sim_params, training_params, nx, delta=True):
    ...
    trajectory_a = device_put(
        jnp.asarray(f["a"][: training_params.n_data]), jax.devices()[0]
    )
    trajectory_t = device_put(
        jnp.asarray(f["t"][: training_params.n_data]), jax.devices()[0]
    )
    trajectory_key = device_put(
        jnp.asarray(f["key"][: training_params.n_data]), jax.devices()[0]
    )
...
def get_loss_fn(model, core_params, forcing_fn, delta=True):
    if delta:
        ...
        f_dadt = time_derivative_FV_1D_burgers_train(
            core_params, model=model, delta=delta
        )
