...
def get_model(core_params, stencil_params, delta=True, diffusion=False):
    ...
    if diffusion == False:
        return LearnedStencil(
            features,
            stencil_params.kernel_size,
            stencil_params.kernel_out,
            stencil_params.stencil_width,
            delta,
        )
    else:
        return LearnedStencilDiffusion(
            features,
            stencil_params.kernel_size,
            stencil_params.kernel_out,
            stencil_params.stencil_width,
...
kwargs_core_learned_diffusion = {
    "Lx": 2 * jnp.pi,
    "flux": "learned_diffusion",
    "nu": nu,
}
...
for i, nx in enumerate(nxs):
    ...
    losses, params = train_model(
        model, i_params, training_params(nx), key, idx_fn, batch_fn, loss_fn
    )
...
for n in range(N):
    ...
    for i, nx in enumerate(nxs):
        ...
        step_fn_model = lambda a, t, dt: sim_model.step_fn(
            a, t, dt, forcing_func=f_forcing
        )
        ...
        step_fn_model_gs = lambda a, t, dt: sim_model_gs.step_fn(
            a, t, dt, forcing_func=f_forcing
        )
