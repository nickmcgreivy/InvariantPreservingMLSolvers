...
labels = ["Centered", "Upwind", "MUSCL MC Limiter", "ML", "ML Upwind-Biased", "ML MC Limiter", "ML Invariant-\nPreserving"]
...
for k in range(7):
    plt.loglog(nxs_rev, (mses[:,k]), label = labels[k], color=colors[k], linewidth=linewidth, linestyle=linestyles[k], marker=markers[k], markersize=8)
