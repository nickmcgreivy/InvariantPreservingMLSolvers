    ...
    if core_params.bc == BoundaryCondition.GHOST:
        ...
        def dadt(a):
            ...
            if G is not None:
                ...
                deta_dt_new = (
                    deta_dt_ratio * deta_dt_old
                )  # + jnp.sum(F[:, -1] * w[:, -1]) - jnp.sum(F[:, 0] * w[:, 0])
        ...
        def dadt(a):
            ...
            if G is not None:
                ...
                diff_w = w[:, 1:] - w[:, :-1]
                deta_dt_old = (
                    jnp.sum(F[:, 1:-1] * diff_w)
                    - jnp.sum(F[:, -1] * w[:, -1])
                    + jnp.sum(F[:, 0] * w[:, 0])
                )
                deta_dt_bc = get_entropy_flux(
                    get_u_left(core_params), core_params
                ) - get_entropy_flux(get_u_right(core_params), core_params)
