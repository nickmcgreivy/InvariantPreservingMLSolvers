...
def create_dataset(args, data_dir, unique_id, nx, ny, nt):
    ...
    dset_a_new = f.create_dataset("a_data", (nt, nx, ny, 1), dtype="float64")
    f.close()
...
def generate_eval_data(args, data_dir, a0, nt, dt, flux, dldt=None):
    ...
    f_poisson_solve = get_poisson_solver(args.poisson_dir, nx, ny, args.Lx, args.Ly, 0)
...
def main():
    ...
    exact_data, dldt_exact = generate_exact_data(
        args, data_dir, a0_exact, nt_one_exact, dt_exact, Tf, flux_exact
    )
    ...
    conservation_data = generate_eval_data(
        args,
        data_dir,
        a0,
        nt,
        dt,
        Flux.CONSERVATION,
        dldt=np.zeros(dldt_exact_ds.shape),
    )
    conservation_dldtexact_data = generate_eval_data(
        args, data_dir, a0, nt, dt, Flux.CONSERVATION, dldt=dldt_exact_ds
    )
    energyconservation_data = generate_eval_data(
        args, data_dir, a0, nt, dt, Flux.ENERGYCONSERVATION
    )
    energyconservation_dldtexact_data = generate_eval_data(
        args, data_dir, a0, nt, dt, Flux.ENERGYCONSERVATION, dldt=dldt_exact_ds
    )
