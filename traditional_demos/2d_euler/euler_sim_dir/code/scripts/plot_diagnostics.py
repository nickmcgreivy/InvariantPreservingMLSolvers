...
for k in range(N):
    ...
    ### energy
    f_poisson = vmap(
        jit(get_poisson_solver(args.poisson_dir, nxs[k], nxs[k], args.Lx, args.Ly, 0))
    )
    H = f_poisson(a_data)
    energy = get_energy(H)
    ...
    if k == 0:
        ...
        M = np.concatenate(
            [
                a_exact_ds[..., 0].reshape(nt, -1)[..., None],
                a_data_ds[..., 0].reshape(nt, -1)[..., None],
            ],
            axis=-1,
        )
    else:
