...
def time_derivative_euler(zeta, t, dx, dy, f_poisson_bracket, f_phi, flux, dldt = None):
    ...
    if flux == Flux.CONSERVATION:
        G_x = np.roll(zeta, -1, axis=0) - zeta
        G_y = np.roll(zeta, -1, axis=1) - zeta
        dldt_old_x = dy * np.sum(F_R * G_x)
        dldt_old_y = dx * np.sum(F_T * G_y)
        if dldt is None:
        ...
        denom_x = dy * np.sum(G_x * G_x)
        denom_y = dx * np.sum(G_y * G_y)

        F_R = F_R + (dldt_new_x - dldt_old_x) * G_x / denom_x
        F_T = F_T + (dldt_new_y - dldt_old_y) * G_y / denom_y

    pb_term = - (F_R - np.roll(F_R, 1, axis=0)) / (dx * dy) - (F_T - np.roll(F_T, 1, axis=1)) / (dx * dy)
    ...
    if flux == Flux.ENERGYCONSERVATION:
        ...
        if dldt is None:
            dldt = dldt_old
        pb_term = P + (dldt - dldt_old) * G / np.sum(W * G)
