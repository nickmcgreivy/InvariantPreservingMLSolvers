...
def get_polynomial_elements_right(order):
    ...
    return Matrix([x ** int(v[k, 0]) * y ** int(v[k, 1]) for k in range(NumElements)])
...
def get_polynomial_elements_top(order):
    ...
    y = symbols("y")
    NumElements = v.shape[0]
    return Matrix([x ** int(v[k, 1]) * y ** int(v[k, 0]) for k in range(NumElements)])
...
def get_R_right(order):
    ...
    for k in range(K, 2 * K):
        for l in range(2 * K):
            expr = (
                legendre_basis[k - K].subs("x", 2 * symbols("x") - 1)
                * recovery_basis[l]
            )
...
def get_R_top(order):
    ...
    for k in range(K, 2 * K):
        for l in range(2 * K):
            expr = (
                legendre_basis[k - K].subs("y", 2 * symbols("y") - 1)
                * recovery_basis[l]
            )
...
def get_A_right(order):
    ...
    for k in range(K):
        for l in range(2 * K):
            expr = legendre_basis[k].subs("x", 1) * diff(recovery_basis[l], "x").subs(
                "x", 0
            ) - 2 * diff(legendre_basis[k], "x").subs("x", 1) * recovery_basis[l].subs(
                "x", 0
            )
...
def get_A_top(order):
    ...
    for k in range(K):
        for l in range(2 * K):
            expr = legendre_basis[k].subs("y", 1) * diff(recovery_basis[l], "y").subs(
                "y", 0
            ) - 2 * diff(legendre_basis[k], "y").subs("y", 1) * recovery_basis[l].subs(
                "y", 0
            )
    ...
    for k in range(K):
        for l in range(K):
            expr = (
                legendre_basis[k]
                * (
                    diff(diff(legendre_basis[l], "x"), "x") * (Ly / Lx)
                    + diff(diff(legendre_basis[l], "y"), "y")
                )
                * (Lx / Ly)
            )
