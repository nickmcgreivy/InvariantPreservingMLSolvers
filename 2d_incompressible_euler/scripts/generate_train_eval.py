...
readwritedir = (
    "/scratch/gpfs/mcgreivy/InvariantPreservingMLSolvers/2d_incompressible_euler"
)
...
def get_sim_params(nx, ny, global_stabilization=False, energy_conserving=False):
    rk = "ssp_rk3"
    flux = Flux.VANLEER
    return FiniteVolumeSimulationParams(
        simname,
        basedir,
        readwritedir,
        nx,
...
def get_simulation(sim_params, model=None, params=None):
    return KolmogorovFiniteVolumeSimulation(
        sim_params, viscosity, forcing_coeff, drag, model=model, params=params
    )
...
def get_ml_params():
    return ModelParams(
        train_id, batch_size, learning_rate, num_epochs, kernel_size, depth, width
    )
...
def plot_fv(zeta, sim_params):
    ...
    xarray.DataArray(zeta, dims=["x", "y"], coords=coords).plot.imshow(
...
def compute_correlation(trajectory_exact, trajectory, sim_params):
    trajectory_exact_ds = jax.vmap(convert_FV_representation, in_axes=(0, None))(
        trajectory_exact, sim_params
    )
    nt = trajectory.shape[0]
    M = jnp.concatenate(
        [
            trajectory.reshape(nt, -1)[:, None, :],
            trajectory_exact_ds.reshape(nt, -1)[:, None, :],
        ],
        axis=1,
    )
...
def correlations(key):
    ...
    inner_fn_burnin = get_inner_fn(
        simulation_exact.step_fn, simulation_exact.dt_fn, t_burnin
    )
    ...
    for i, nx in enumerate(nxs):
        ...
        # Model EC
        sim_params_ec = get_sim_params(
            nx, nx, global_stabilization=True, energy_conserving=True
        )
        ...
        corrs_vanleer[i] = compute_correlation(
            exact_trajectory, traj_vanleer, sim_params
        )
        corrs_model[i] = compute_correlation(exact_trajectory, traj_model, sim_params)
        corrs_model_gs[i] = compute_correlation(
            exact_trajectory, traj_model_gs, sim_params
        )
        corrs_model_ec[i] = compute_correlation(
            exact_trajectory, traj_model_ec, sim_params
        )
...
for i, nx in enumerate(nxs):
    ...
    ax.plot(
        Ts[:tf],
        jnp.ones(Ts.shape)[:tf],
        color="#008000",
        label="MUSCL {}x{}".format(nx_exact, nx_exact),
        linewidth=lw,
    )
    ax.plot(
        Ts[:tf],
    ...
    ax.plot(
        ...
        color="red",
        linestyle="dotted",
        label="ML {}x{}\n$\ell_2$-decaying &\nEnergy Conserving".format(nx, nx),
        linewidth=lw,
    )
