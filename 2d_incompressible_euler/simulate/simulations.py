from poissonbracket import get_poisson_bracket_fn_fv, load_alpha_right_matrix_twice, load_alpha_top_matrix_twice
...
class KolmogorovFiniteVolumeSimulation():
    ...
    def __init__(self, sim_params, viscosity, forcing_coeff, drag, model=None, params=None):
    ...
    def get_step_fn(self, sim_params, model=None, params=None):
        ...
        dadt = time_derivative_fn_fv(sim_params, poisson_bracket_fn, self.poisson_solve_fn, forcing_fn=forcing_fn, diffusion_fn=diffusion_fn)
    ...
    def get_dt_fn(self, sim_params):
        ...
        def get_dt(a):
            ...
            max_R_new = jnp.amax(jnp.abs(jnp.roll(a, 1, axis=1) - a)) * sim_params.dx * 2
            max_T_new = jnp.amax(jnp.abs(jnp.roll(a, 1, axis=0) - a)) * sim_params.dy * 2

            max_R = jnp.maximum(max_R_standard, max_R_new)
            max_T = jnp.maximum(max_T_standard, max_T_new)
            return sim_params.cfl_safety * (sim_params.dx * sim_params.dy) / (max_R * sim_params.dy + max_T * sim_params.dx)
